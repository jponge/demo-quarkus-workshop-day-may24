...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      refreshData() {
        axios
          .get(`http://localhost:4000/${DataStore.username()}`, {
            headers: {
        ...
        axios
          .get(`http://localhost:4000/${DataStore.username()}/total`, {
            headers: {
              'Authorization': `Bearer ${DataStore.token()}`
            }
          })
        ...
        axios
          .get(`http://localhost:4000/${DataStore.username()}/${today.getFullYear()}/${today.getUTCMonth() + 1}`, {
            headers: {
              'Authorization': `Bearer ${DataStore.token()}`
            }
          })
        ...
        axios
          .get(`http://localhost:4000/${DataStore.username()}/${today.getFullYear()}/${today.getUTCMonth() + 1}/${today.getDate()}`, {
            headers: {
              'Authorization': `Bearer ${DataStore.token()}`
            }
          })
      ...
      sendUpdate() {
        ...
        axios
          .put(`http://localhost:4000/${DataStore.username()}`, data, config)
